...
describe('release publishable libraries in workspace with ts solution setup', () => {
  ...
  beforeAll(async () => {
    newProject({
      packages: ['@nx/js', '@nx/react', '@nx/react-native'],
      preset: 'ts',
    });
  ...
  it('should be able to release publishable react native library', async () => {
    ...
    expect(releaseOutput).toMatchInlineSnapshot(`
      NX   Executing pre-version command
      NX   Running release version for project: @test/{project-name}
      @test/{project-name} 🔍 Reading data for package "@test/{project-name}" from packages/{project-name}/package.json
      @test/{project-name} 📄 Resolved the current version as 0.0.1 from packages/{project-name}/package.json
      @test/{project-name} 📄 Using the provided version specifier "0.0.6".
      @test/{project-name} ✍️  New version 0.0.6 written to packages/{project-name}/package.json
      ...
      NX   Committing changes with git
      NX   Tagging commit with git
      NX   Running target nx-release-publish for project @test/{project-name}:
      - @test/{project-name}
      > nx run @test/{project-name}:nx-release-publish
      📦  @test/{project-name}@0.0.6
      === Tarball Contents ===
      XXB README.md
      XXB dist/index.esm.d.ts
      XXB dist/index.esm.js
      ...
      name:          @test/{project-name}
      version:       0.0.6
      filename:      test-{project-name}-0.0.6.tgz
      package size:  XXX.XXX kb
      unpacked size: XXX.XXX kb
      shasum:        {SHASUM}
      integrity: XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
      total files: X
      Published to ${e2eRegistryUrl} with tag "latest"
      NX   Successfully ran target nx-release-publish for project @test/{project-name}
    `);
  });
});
